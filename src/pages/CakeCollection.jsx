import { useState } from 'react'
import { Link } from 'react-router-dom'
import { motion } from 'framer-motion'
import { Search, SlidersHorizontal } from 'lucide-react'
import cakes, { categories } from '../data/cakes'
import FadeInSection from '../components/FadeInSection'
import StarRating from '../components/StarRating'

export default function CakeCollection() {
  const [activeCategory, setActiveCategory] = useState('all')
  const [searchQuery, setSearchQuery] = useState('')
  const [sortBy, setSortBy] = useState('featured')

  let filtered = activeCategory === 'all'
    ? cakes
    : cakes.filter((c) => c.category === activeCategory)

  if (searchQuery) {
    filtered = filtered.filter((c) =>
      c.name.toLowerCase().includes(searchQuery.toLowerCase()) ||
      c.description.toLowerCase().includes(searchQuery.toLowerCase())
    )
  }

  if (sortBy === 'price-low') filtered = [...filtered].sort((a, b) => a.price - b.price)
  if (sortBy === 'price-high') filtered = [...filtered].sort((a, b) => b.price - a.price)
  if (sortBy === 'rating') filtered = [...filtered].sort((a, b) => b.rating - a.rating)

  return (
    <>
      {/* Hero */}
      <section className="relative pt-32 pb-16 overflow-hidden">
        <div className="absolute inset-0 bg-gradient-to-br from-cream via-blush/20 to-cream-dark" />
        <div className="absolute top-20 right-10 w-80 h-80 bg-gold/10 rounded-full blur-3xl" />

        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <span className="text-xs font-medium text-gold uppercase tracking-[0.2em] mb-4 block">
            Our Creations
          </span>
          <h1 className="font-display text-5xl sm:text-6xl font-bold text-chocolate mb-6">
            Cake Collection
          </h1>
          <p className="text-lg text-charcoal/60 max-w-2xl mx-auto">
            From timeless wedding tiers to playful birthday showstoppers,
            discover our curated collection of signature cakes.
          </p>
        </div>
      </section>

      {/* Filters & Search */}
      <section className="py-6 sticky top-20 z-40 bg-cream/90 backdrop-blur-md border-b border-cream-dark/50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4">
            {/* Categories */}
            <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-hide flex-1">
              {categories.map((cat) => (
                <button
                  key={cat.id}
                  onClick={() => setActiveCategory(cat.id)}
                  className={`flex-shrink-0 px-5 py-2.5 rounded-full text-sm font-medium transition-all duration-300 ${
                    activeCategory === cat.id
                      ? 'bg-chocolate text-white shadow-md'
                      : 'bg-white text-charcoal/60 hover:text-chocolate hover:bg-cream-dark'
                  }`}
                >
                  {cat.name}
                  <span className="ml-1.5 text-xs opacity-60">({cat.count})</span>
                </button>
              ))}
            </div>

            {/* Search & Sort */}
            <div className="flex items-center gap-3 w-full sm:w-auto">
              <div className="relative flex-1 sm:flex-initial">
                <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-warm-gray" />
                <input
                  type="text"
                  placeholder="Search cakes..."
                  value={searchQuery}
                  onChange={(e) => setSearchQuery(e.target.value)}
                  className="w-full sm:w-48 pl-9 pr-3 py-2.5 rounded-xl border border-cream-dark bg-white text-sm text-charcoal focus:outline-none focus:ring-2 focus:ring-gold/30 transition-all"
                />
              </div>
              <div className="relative">
                <SlidersHorizontal className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-warm-gray" />
                <select
                  value={sortBy}
                  onChange={(e) => setSortBy(e.target.value)}
                  className="pl-9 pr-8 py-2.5 rounded-xl border border-cream-dark bg-white text-sm text-charcoal focus:outline-none focus:ring-2 focus:ring-gold/30 appearance-none cursor-pointer"
                >
                  <option value="featured">Featured</option>
                  <option value="price-low">Price: Low to High</option>
                  <option value="price-high">Price: High to Low</option>
                  <option value="rating">Top Rated</option>
                </select>
              </div>
            </div>
          </div>
        </div>
      </section>

      {/* Grid */}
      <section className="py-16">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          {filtered.length === 0 ? (
            <div className="text-center py-20">
              <div className="text-5xl mb-4">🎂</div>
              <h3 className="font-display text-xl font-semibold text-chocolate mb-2">No cakes found</h3>
              <p className="text-warm-gray text-sm">Try adjusting your filters or search query.</p>
            </div>
          ) : (
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-8">
              {filtered.map((cake, i) => (
                <FadeInSection key={cake.id} delay={Math.min(i * 80, 400)}>
                  <motion.div
                    whileHover={{ y: -8 }}
                    transition={{ duration: 0.3 }}
                    className="group bg-white rounded-3xl overflow-hidden shadow-sm hover:shadow-xl transition-all duration-500 h-full flex flex-col"
                  >
                    <Link to={`/cake/${cake.id}`} className="block">
                      <div className="relative h-64 overflow-hidden">
                        <img
                          src={cake.image}
                          alt={cake.name}
                          className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110"
                          loading="lazy"
                        />
                        <div className="absolute inset-0 bg-gradient-to-t from-chocolate/40 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500" />
                        <div className="absolute top-4 left-4 flex gap-2">
                          {cake.badge && (
                            <span className="bg-gold text-white text-[10px] font-semibold uppercase tracking-wider px-3 py-1.5 rounded-full">
                              {cake.badge}
                            </span>
                          )}
                          {cake.originalPrice && (
                            <span className="bg-rose text-white text-[10px] font-semibold px-3 py-1.5 rounded-full">
                              Sale
                            </span>
                          )}
                        </div>
                      </div>
                    </Link>
                    <div className="p-6 flex flex-col flex-1">
                      <span className="text-[10px] uppercase tracking-wider text-gold font-medium mb-1">
                        {cake.category}
                      </span>
                      <div className="flex items-start justify-between mb-2">
                        <Link to={`/cake/${cake.id}`}>
                          <h3 className="font-display text-xl font-semibold text-chocolate group-hover:text-gold transition-colors duration-300">
                            {cake.name}
                          </h3>
                        </Link>
                        <div className="text-right flex-shrink-0 ml-4">
                          <span className="font-display text-lg font-semibold text-gold block">${cake.price}</span>
                          {cake.originalPrice && (
                            <span className="text-xs text-warm-gray line-through">${cake.originalPrice}</span>
                          )}
                        </div>
                      </div>
                      <p className="text-sm text-warm-gray leading-relaxed mb-4 flex-1">
                        {cake.shortDescription}
                      </p>
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          <StarRating rating={cake.rating} size={12} />
                          <span className="text-xs text-warm-gray">({cake.reviewCount})</span>
                        </div>
                        <div className="flex items-center gap-3 text-xs text-warm-gray">
                          <span>Serves {cake.serves}</span>
                        </div>
                      </div>
                      <Link
                        to={`/cake/${cake.id}`}
                        className="block w-full text-center py-3 mt-4 rounded-xl bg-cream hover:bg-chocolate hover:text-white text-sm font-medium text-chocolate transition-all duration-300"
                      >
                        View Details
                      </Link>
                    </div>
                  </motion.div>
                </FadeInSection>
              ))}
            </div>
          )}
        </div>
      </section>

      {/* CTA */}
      <section className="py-16 bg-white/50">
        <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <FadeInSection>
            <h3 className="font-display text-2xl font-bold text-chocolate mb-4">
              Can't Find What You're Looking For?
            </h3>
            <p className="text-warm-gray mb-8 leading-relaxed">
              Every cake we make can be customized to your exact specifications.
              Flavors, colors, sizes, and designs — nothing is set in stone.
            </p>
            <Link
              to="/custom-orders"
              className="inline-flex items-center gap-2 bg-chocolate text-white px-8 py-4 rounded-full text-sm font-medium hover:bg-chocolate-light transition-all duration-300 hover:shadow-xl hover:-translate-y-0.5"
            >
              Design a Custom Cake
            </Link>
          </FadeInSection>
        </div>
      </section>
    </>
  )
}
