              ...
              <div className="mb-6">
                ...
                <div className="flex flex-wrap gap-2">
                  {cake.flavors.map((flavor) => (
                    <span key={flavor} className="px-4 py-2 rounded-full bg-white text-sm text-charcoal/70 border border-cream-dark">
                      {flavor}
                    </span>
                  ))}
                </div>
                ...
                >
                  {addedToCart ? (
                    ...
                    <>
                      ...
                      Add to Cart — ${cake.price * quantity}
                    </>
                  )}
                </motion.button>
              </div>
              ...
              <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-8">
                {related.map((item, i) => (
                  <FadeInSection key={item.id} delay={i * 100}>
                    <Link
                      to={`/cake/${item.id}`}
                      className="group bg-white rounded-3xl overflow-hidden shadow-sm hover:shadow-xl transition-all duration-500 hover:-translate-y-1 block"
                    >
